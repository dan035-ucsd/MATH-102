...
\subsection{Theorem 9.3.3}

Let $A\in \mathbb{M}_n$ and let $p$ be a nonconstant polynomial that annihilates $A$. 
Every eigenvalue of $A$ is a root of $p(z)=0$. \\ 

\noindent \textbf{Proof:} 

Let $(\lambda,\mathbf{x})$ be an eigenpair of $A$. Then $\mathbf{0} = 0_n\mathbf{x}=p(A)\mathbf{x} = p(\lambda)\mathbf{x}$, so $p(\lambda)\mathbf{x} = 0$


\subsubsection{Is The Converse True?}
...
Let $(\lambda,x)$ be an eigenpair of $A\in \mathbb{M}_n$ and let $p$ be a polynomial.
Then $p(A)x=p(\lambda)x$, that is, $(p(\lambda),x)$ is an eigenpair of $p(A)$. \\

\noindent \textbf{Proof:} 

Recall $A^k\textbf x = \lambda^k \textbf x$.
\begin{align*}
    p(A)\textbf x &= (c_k A^k + c_{k-1} A^{k-1} + ... + c_1A + c_0I)\textbf x \\
    &= c_k A^k \textbf x + c_{k-1} A^{k-1} \textbf x+ ... + c_1A \textbf x + c_0I\textbf x \\
    &= c_k \lambda^k \textbf x + c_{k-1} \lambda^{k-1} \textbf x+ ... + c_1\lambda \textbf x + c_0\textbf x \\
    &= (c_k \lambda^k + c_{k-1} \lambda^{k-1} + ... + c_1\lambda + c_0)\textbf x \\
    &= p(\lambda)\textbf x 
\end{align*}

\subsection{2-Commute}
Let $A_1, A_2 \in \mathbb{M}_n$. Let $\lambda$ be an eigenvalue of $A_1$. Suppose $A_1A_2=A_2A_1$, then $A_1,A_2$ share an eigenvector in $\mathcal{E}_\lambda (A_1)$ \\
...
If $n \ge 2$ and $A=\begin{bmatrix}
    a_{ij}
\end{bmatrix} \in \mathbb{M}_n$ then
\begin{align*}
    \text{spec } A \subseteq \mathcal{G}(A)= \bigcup^n_{k=1} \mathcal{G}_k(A).
\end{align*}

\noindent\textbf{Proof:}

Let $(\lambda,\mathbf{x})$ be an eigenpair of $A = \begin{bmatrix}
a_{ij} 
\end{bmatrix} \in \mathbb{M}_n$ and let $\mathbf{x}=\begin{bmatrix}
    x_i
\end{bmatrix}$. Since $\lambda \mathbf{x}=A\mathbf{x}$, we have
\begin{align*}
    \lambda x_i = \displaystyle\sum^n_{j=1} a_{ij}x_j &= a_{ii}x_i+\displaystyle\sum_{j\ne i} a_{ij}x_j \\
    (\lambda - a_{ii})x_i &= \displaystyle\sum_{j\ne i} a_{ij}x_j 
\end{align*}

Let $k \in \{ 1,2,...,n \}$ be any index such that $|x_k|=\max_{1\le i \le n} |x_i|$. Since $\mathbf{x} \ne \mathbf{0}$, 
it follows that $|x_k|>0$ and $\frac{|x_j|}{x_k}\le 1$ for all $i = 1,2,...,n$. Set $i = k$ and divide by $x_k$ to obtain
\begin{align*}
    \lambda - a_{kk} = \displaystyle\sum_{j\ne k} a_{kj} \frac{x_j}{x_k}
\end{align*}

By the triangle inequality,
\begin{align*}
    |\lambda-a_{kk}|=\left| \displaystyle\sum_{j\ne k}a_{kj}\frac{x_j}{x_k} \right| \le \sum_{j\ne k} |a_{kj}| \left| \frac{x_j}{x_k} \right| \le \sum_{j\ne k} |a_{kj}| = R'_k(A)
\end{align*}

Therefore, $\lambda \in \mathcal{G}_k(A)\subseteq \mathcal{G}(A)$

\subsection{Corollary 9.4.15}
Let $A\in \mathbb{M}_n$. If $A$ is strictly diagonally dominant, then it is invertible. \\

\noindent \textbf{Proof: }

The strict diagonal dominance of $A$ ensures that $0\notin\mathcal{G}_k(A)$ for each $k=1,2,...,n$. 
The Gershgorin Disk Theorem then tells us that 0 is not an eigenvalue of $A$ and therefore $A$ is invertible.

\section{Similar Matrices}
\subsection{Theorem 9.6.3}
If two real matrices are similar, then they are similar via a real matrix. \\

\noindent \textbf{Proof:}

Let $A,B \in \mathbb{M}_n(\mathbb{R})$. Let $S\in \mathbb{M}_n$ be invertible such that $A=SBS^{-1}$.
For any $\theta \in (-\pi, \pi ],$ let $S_\theta = e^{i\theta}S$ and compute $A = (e^{i\theta}S)B(e^{-i\theta}S^{-1})=S_\theta B S_\theta ^{-1}$.
Then $AS_\theta = S_\theta B$. The complex conjugate of this identity is $A\bar{S_\theta}=\bar{S_\theta}B$ since $A$ and $B$ are real.
Let $R_\theta=S_\theta+\bar{S_\theta} = 2\text{ Re } S_\theta $ which is real for all $\theta\in (-\pi,\pi]$. 
Then $AR_\theta = A(S_\theta + \bar{S_\theta})=(S_\theta + \bar{S_\theta})B=R_\theta B$. The computation 
\begin{align*}
    R_\theta = S_\theta(I+S_\theta^{-1}\bar{S_\theta}) = S_\theta (I+e^{-2i\theta}S^{-1}\bar S)= e^{-2i\theta}S_\theta(e^{2i\theta}I+S^{-1}\bar S)
\end{align*}

\noindent shows that $R_\theta$ is invertible if $-e^{-2i\theta}$ is not an eigenvalue of $S^{-1}\bar S$. Since $S^{-1}\bar S$ has at most $n$ distinct eigenvalues, 
there is a $\phi$ such that $-e^{-2i\phi}$ is not an eigenvalue of $S^{-1}\bar S$. Then $AR_\phi = R_\phi B$, where $R_\phi$ is real and invertible, and $A=R_\phi BR_\phi^{-1}$.
